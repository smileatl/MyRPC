...
void RpcProvider::Run() {
    ...
    uint16_t port = atoi(MprpcApplication::GeitInstance()
                             ...
                             .Load("rpcserverport")
                             .c_str());
    mynetlib::InetAddress address(ip, port);

    // 创建TcpServer对象
    mynetlib::TcpServer server(&m_eventLoop, address, "RpcProvider");
...
// 新的socket连接回调
void RpcProvider::OnConnection(const mynetlib::TcpConnectionPtr& conn) {
    if (!conn->connected()) {
        // 和rpc client的连接断开了
        conn->shutdown();
    }
...
void RpcProvider::OnMessage(const mynetlib::TcpConnectionPtr& conn,
                            mynetlib::Buffer* buffer,
                            mynetlib::Timestamp) {
    ...
    google::protobuf::Closure* done =
        google::protobuf::NewCallback<RpcProvider,
                                      const mynetlib::TcpConnectionPtr&,
...
void RpcProvider::SendRpcResponse(const mynetlib::TcpConnectionPtr& conn,
                                  google::protobuf::Message* response) {
